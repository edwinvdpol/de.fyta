...
class Device extends OAuth2Device {
  ...
  */
  ...
  async migrate() {
    ...
    // Add 'measure_humidity' capability
    if (!this.hasCapability('measure_humidity')) {
      await this.addCapability('measure_humidity');
      this.log('[Migrate] Added `measure_humidity` capability');
    }

    // Add 'status_humidity' capability
    if (!this.hasCapability('status_humidity')) {
      await this.addCapability('status_humidity');
      this.log('[Migrate] Added `status_humidity` capability');
    }
