...
class Data {
  ...
  get valid() {
    return filled(this.id) && this.status !== 0 && this.has_sensor === true;
  }
  ...
  fromApiData(raw) {
    ...
    if ('measurements' in raw && filled(raw.measurements)) {
      let msts = raw.measurements;
      if ('air_humidity' in msts) data.measure_humidity = this.measureValue('air_humidity', msts);
      if ('air_humidity' in msts) data.status_humidity = this.measureStatus('air_humidity', msts);
    ...
    if ('sensor' in raw && filled(raw.sensor)) {
